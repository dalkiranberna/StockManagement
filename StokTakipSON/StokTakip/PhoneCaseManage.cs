...
namespace StokTakip
{
    public partial class PhoneCaseManage : Form
    {
        ...
        private void btn_phoneCaseAdd_Click(object sender, EventArgs e)
        {
            if(dataGridView1.SelectedRows.Count == 0)
                new PhoneCaseCreate().Show();
            else
            {
                int chosenId = (int)dataGridView1.SelectedRows[0].Cells["ID"].Value;
